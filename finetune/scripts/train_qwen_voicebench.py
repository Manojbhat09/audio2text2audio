#!/usr/bin/env python3
"""
Full fine-tuning script for Qwen model with VoiceBench dataset for dataset classification.
"""
...
class VoiceBenchDataset(Dataset):
    """Dataset for VoiceBench classification training."""
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if transcript.startswith("Error"):
                ...
                transcript = sample.get('text', 'No transcript available')
            
            # Create training prompt
            prompt = self._create_training_prompt(transcript, dataset_name)
            ...
            dummy_text = "This is a dummy sample for training."
            inputs = self.tokenizer(
                dummy_text,
                max_length=self.max_length,
                padding='max_length',
    ...
    def _create_training_prompt(self, transcript, dataset_name):
...
Your task is to classify transcripts into one of these categories:
...
Which dataset category does this transcript most likely come from? Answer with only the dataset name: ifeval, commoneval, wildvoice, or other.

Answer: {dataset_name}"""

def main():
    parser = argparse.ArgumentParser(description='Train Qwen model on VoiceBench dataset')
    ...
    parser.add_argument('--output-dir', type=str, default='./qwen_outputs',
                       help='Output directory')
    parser.add_argument('--model-dir', type=str, default='./qwen_models',
                       help='Model save directory')
    
    args = parser.parse_args()
    
    logger.info("=" * 60)
    logger.info("QWEN VOICEBENCH TRAINING CONFIGURATION")
    logger.info("=" * 60)
    ...
    # Create dataset
    logger.info("Creating VoiceBench dataset...")
    try:
        dataset = VoiceBenchDataset(
            tokenizer=tokenizer,
    ...
    # Save model
    model_save_path = Path(args.model_dir) / "final_qwen_model"
    ...
    test_prompts = [
        "Transcript: 'Please follow these instructions step by step.' Answer:",
        "Transcript: 'This is a common evaluation task.' Answer:",
        "Transcript: 'Hello, this is a natural conversation.' Answer:",
    ]
