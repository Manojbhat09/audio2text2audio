#!/usr/bin/env python3
"""
Test the LoRA-trained Qwen model on classification performance.
"""
...
class LoRAClassificationTester:
    """Test the LoRA-trained model on classification tasks."""
    
    def __init__(self, base_model_path: str, lora_model_path: str, max_length: int = 256):
    ...
    def create_test_prompt(self, text: str) -> str:
        ...
        return f"Classify this text as ifeval, commoneval, or wildvoice:\n\nText: {text}\n\nClassification:"
    ...
    def run_comprehensive_test(self) -> Dict[str, Any]:
        ...
        logger.info("Starting comprehensive LoRA classification test...")
    ...
    def print_results(self, results: Dict[str, Any]):
        ...
        logger.info("\n" + "="*60)
        logger.info("LORA CLASSIFICATION TEST RESULTS")
        logger.info("="*60)
...
def main():
    parser = argparse.ArgumentParser(description='Test LoRA-trained Qwen model on classification')
    parser.add_argument('--base-model-path', type=str, 
                       default='Qwen/Qwen2.5-0.5B-Instruct',
                       help='Path to base model')
    parser.add_argument('--lora-model-path', type=str, 
                       default='./qwen_lora_models/final_qwen_lora_model',
                       help='Path to LoRA model')
    parser.add_argument('--max-length', type=int, default=256,
                       help='Maximum sequence length')
    parser.add_argument('--output-file', type=str, default='lora_classification_test_results.json',
    ...
    # Initialize tester
    tester = LoRAClassificationTester(args.base_model_path, args.lora_model_path, args.max_length)
    ...
    if results['overall_accuracy'] >= 0.8:
        logger.info("🎉 Excellent performance! The LoRA model is learning very well.")
    elif results['overall_accuracy'] >= 0.6:
        logger.info("👍 Good performance! The LoRA model is learning but could be improved.")
    elif results['overall_accuracy'] >= 0.4:
        logger.info("⚠️  Moderate performance. The LoRA model needs more training.")
    else:
        logger.info("❌ Poor performance. The LoRA model is not learning the task effectively.")
