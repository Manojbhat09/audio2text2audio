...
async def main(config: bt.config):
    ...
    def get_latest_commit(repo_id: str) -> str:
        """Get the latest commit hash from a Hugging Face repository."""
        api = HfApi()
        commits = list_repo_commits(repo_id=repo_id, repo_type="model")
        if commits:
            return commits[0].commit_id
        else:
            raise ValueError(f"No commits found for repository {repo_id}")

    # Get the latest commit from Hugging Face instead of uploading
    try:
        latest_commit = get_latest_commit(f"{repo_namespace}/{repo_name}")
        print(f"Latest commit for {repo_namespace}/{repo_name}: {latest_commit}")
    ...
    model_hash = regenerate_hash(str(repo_namespace), str(repo_name), str(config.epoch), config.competition_id)
    model_id_with_hash = ModelId(
        namespace=repo_namespace,
        name=repo_name,
        epoch=str(config.epoch),
        hash=str(model_hash),
        commit=str(latest_commit),
        competition_id=config.competition_id,
    )
    ...
    while True:
        try:
            ...
            # Skip updating repo settings since repository already exists and is accessible
            print("Committed model to the chain.")
            break
        except Exception as e:
            print(f"Failed to advertise model on the chain: {e}")
